...
The API now exposes a stateless, authenticated MCP server at `/mcp`. This lets ChatGPT read bounded Life OS context, assemble a dated morning brief, preview changes, save idempotent batches and undo a requested batch within 30 days. It uses ChatGPT's inference and does not need a separate model API key. The website remains usable without this connection.

Set `ASSISTANT_PUBLIC_URL` to the canonical HTTPS API origin (default for this deployment: `https://life-os-api-six.vercel.app`). Keep the existing `ENCRYPTION_KEY` stable. Apply `alembic upgrade head` before deploying. Migration `0003` adds encrypted OAuth state, assistant change receipts, learning topics/sessions and course-linked exams. JSON and CSV backups include these tables; earlier `0001` and `0002` backups remain restorable.

In ChatGPT's private plugin setup, use the API origin followed by `/mcp`, choose OAuth, and sign in with the Life OS owner account. The server provides OAuth discovery and dynamic client registration for ChatGPT HTTPS callbacks. PKCE is required; codes are single-use for two minutes, access tokens last one hour, and refresh tokens rotate with a 30-day lifetime. Disconnect in Life OS Settings → ChatGPT & history to revoke access. Do not use your phone webhook key for ChatGPT.

The private Life OS project uses project-only memory and instructions for relevant opt-in tracking. A project instruction is not a technical attachment or a guarantee that ChatGPT invokes a tool; verify the connector is available in each supported client. Ordinary chats should explicitly invoke Life OS. Do not save full transcripts, infer mastery from an explanation, or invent study durations. A journal summary and an AI insight are separate rows. Explicit journal/insight deletion also purges related recoverable assistant history.

Changes use a stable `request_key` for retries. Updates and task/review actions need the current record's `expected_updated_at`. Creation labels may be referenced by later operations as `$label`. Preview rolls back its entire transaction; save is atomic. Undo refuses changed records and later dependencies. Use a stable exam import key to identify repeated timetable imports; courses must already exist or be added in Academics. Existing task recurrence and SM-2 rules are reused.

The morning brief uses dated canonical health records, keeps activity sources separate, and excludes Samsung raw fields with unverified units. A configured preference is not a scheduled task: a cloud ChatGPT schedule must be created and verified separately for 09:00 Asia/Kolkata. Private GPT Actions and optional mobile/account fallbacks are not activated merely by deploying MCP. See `docs/CHATGPT_CONNECTION.md` for verification status and limitations.
