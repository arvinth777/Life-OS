...
Synthetic checks cover the encrypted sign-in exchange, callback replay rejection, resumable pages, duplicate raw stress fields, encrypted backup state and safe failures. Actual Samsung sign-in, cloud authorization, real field semantics and phone hydration still need to be verified. Errors leave saved data usable and do not block the rest of the app. Reconnect if Samsung invalidates the private protocol credentials.
